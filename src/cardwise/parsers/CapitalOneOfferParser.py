...
class CapitalOneOfferParser(BankOfferParser):
    ...
    def _extract_offers(self, soup: BeautifulSoup) -> List[Offer]:
        ...
        if not tiles:
            raise OfferParsingError(self.bank_info.name, "No tile elements found")

        for tile in tiles:
            img_tag = tile.find("img")
            if not isinstance(img_tag, Tag) or not img_tag.has_attr("src"):
                raise OfferShopNameParsingError(
                    self.bank_info.name, "Missing 'src' in image tag, no URL to get shop name"
                )
            ...
            if not match:
                raise OfferShopNameParsingError(self.bank_info.name, f"""Not in expected position at URL: {src_url}""")
            ...
            if not offer_description:
                raise OfferDescriptionParsingError(
                    self.bank_info.name, f"""Missing text in div tag. Div: {offer_divs[1]}"""
                )
            ...
            results.add(
                Offer(
                    ...
                    offer_type=OfferTypeEnum.POINTS,  # Now normalized with "miles" as "points"
