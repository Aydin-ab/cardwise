...
# This lets us skip copying source code and just reuse the venv at runtime
...
# Use clean python-slim image to reduce size, we don't need the other ENV vars
FROM python:3.12-slim AS production

ENV VIRTUAL_ENV="/opt/pysetup/venv"
COPY --from=production-builder ${VIRTUAL_ENV} ${VIRTUAL_ENV}
ENV PATH="$VIRTUAL_ENV/bin:$PATH"
ENTRYPOINT ["search_offers"]
CMD ["--help"]
